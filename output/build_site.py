    ...
    (function(){{
        ...
        function renderEspnBracket(confGames,bracketDef,confSeeds){{
            ...
            for(var mi=0;mi<numMatch;mi++){{
                ...
                /* Split bracket slots into ref (expects TBD) and seeded (both seeds) */
                var refIdxs=[],seedIdxs=[];
                defSlots.forEach(function(sl,idx){{
                    if(sl.top.ref||sl.bot.ref)refIdxs.push(idx);
                    else seedIdxs.push(idx);
                }});

                /* Match known games to seeded slots by seed sum.
                   Each bracket slot has a seed pair (e.g. 1v8=sum9, 4v5=sum9).
                   Use the home team's conf-record seed to find the matching slot. */
                if(knownQ.length>0&&seedIdxs.length>0){{
                    /* For each known game, find the home team's seed from seedMap */
                    var gameSeeds=knownQ.map(function(g,i){{
                        var hs=seedMap[g.homeId]||seedMap[(g.homeName||'').toLowerCase()]||99;
                        return {{gi:i,homeSeed:hs}};
                    }});
                    /* For each seeded slot, find the higher seed (min) */
                    var slotSeeds=seedIdxs.map(function(idx){{
                        var sl=defSlots[idx];
                        return {{idx:idx,highSeed:Math.min(sl.top.seed||99,sl.bot.seed||99)}};
                    }});
                    /* Match: game whose home team seed equals slot's higher seed */
                    var slotGameMap={{}};
                    var usedGames={{}};
                    slotSeeds.forEach(function(ss){{
                        for(var gi=0;gi<gameSeeds.length;gi++){{
                            if(!usedGames[gi]&&gameSeeds[gi].homeSeed===ss.highSeed){{
                                slotGameMap[ss.idx]=knownQ[gi];
                                usedGames[gi]=true;
                                return;
                            }}
                        }}
                    }});
                    /* Fallback: assign remaining games by index */
                    var remaining=[];
                    knownQ.forEach(function(g,i){{if(!usedGames[i])remaining.push(g);}});
                    var ri2=0;
                    seedIdxs.forEach(function(idx){{
                        if(!slotGameMap[idx]&&ri2<remaining.length)slotGameMap[idx]=remaining[ri2++];
                    }});
                    /* Reorder knownQ to follow bracket def slot order */
                    var orderedKnown=[];
                    seedIdxs.forEach(function(idx){{
                        if(slotGameMap[idx])orderedKnown.push(slotGameMap[idx]);
                    }});
                    knownQ=orderedKnown;
                }}

                /* Rebuild game list following bracket def slot order */
                var reordered=[];
                var ti=0,ki=0;
                defSlots.forEach(function(sl){{
                    var hasRef=sl.top.ref||sl.bot.ref;
