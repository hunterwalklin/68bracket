    ...
    (function(){{
        ...
        function renderEspnBracket(confGames,bracketDef,confSeeds){{
            ...
            /* Reorder ESPN games within each round to match bracket def slot pattern.
               ...
               1. TBD/known split: bye games (TBD opponent) match ref slots,
                  known games match seeded slots.
               2. Spread-based: for all-known rounds, match games to slots by
                  predicted spread — largest mismatch → highest seed spread slot. */
            ...
            for(var mi=0;mi<numMatch;mi++){{
                ...
                /* For known games matching seeded slots: sort both by spread
                   to match biggest mismatch → highest seed-spread slot */
                if(knownQ.length>0&&seedIdxs.length>0){{
                    var slotSpread=seedIdxs.map(function(idx){{
                        var sl=defSlots[idx];
                        return {{idx:idx,spread:Math.abs((sl.top.seed||0)-(sl.bot.seed||0))}};
                    }});
                    slotSpread.sort(function(a,b){{return b.spread-a.spread;}});
                    var gameSpread=knownQ.map(function(g,i){{
                        return {{gi:i,spread:g.pred?Math.abs(g.pred.spread):0}};
                    }});
                    gameSpread.sort(function(a,b){{return b.spread-a.spread;}});
                    /* Build assignment: bracket slot index → known game */
                    var slotGameMap={{}};
                    var mc=Math.min(slotSpread.length,gameSpread.length);
                    for(var i=0;i<mc;i++)slotGameMap[slotSpread[i].idx]=knownQ[gameSpread[i].gi];
                    /* Reorder knownQ to follow bracket def order */
                    var orderedKnown=[];
                ...
                /* Assign seeds from bracket def slots to reordered ESPN games */
